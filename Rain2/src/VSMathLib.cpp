...
std::shared_ptr<VSMathLib> VSMathLib::gInstance;
...
std::shared_ptr<VSMathLib> 
    VSMathLib::getInstance(void) {

        if (gInstance)
            return gInstance;
        else
            gInstance = std::shared_ptr<VSMathLib> (new VSMathLib());
        ...
        if (mBlocks) {
            if (mUniformName[aType] != "") {
                if (mUniformArrayIndex[aType]) {
                    VSShaderLib::setBlockUniformArrayElement(
                        mBlockName,
                        mUniformName[aType],
